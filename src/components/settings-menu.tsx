...
export function ExcludedJobsDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const { excludedJobs, addExcludedJob, removeExcludedJob } = useSettings()
  const [input, setInput] = useState('')
  const inputRef = useRef<HTMLInputElement>(null)

  const handleAdd = () => {
    if (!input.trim()) return
    addExcludedJob(input.trim())
    setInput('')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm gap-0 p-7 rounded-2xl" showCloseButton={false}>
        <div className="flex items-center justify-between mb-5">
          <DialogTitle>Excluded Jobs</DialogTitle>
          <button
        ...
        <div className="flex flex-col gap-1.5 mb-5 max-h-44 overflow-y-auto">
          ...
          {excludedJobs.map(job => (
            <div key={job} className="flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-muted group">
              <span className="flex-1 text-sm font-mono truncate">{job}</span>
              <button
                onClick={() => removeExcludedJob(job)}
